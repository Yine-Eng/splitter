...
public class BalanceFlowIntegrationTest {
        ...
        void fullBalanceFlow_withBigDecimalAndUnevenSplit() throws Exception {
                ...
                HttpEntity<String> signupUser1Req = new HttpEntity<>(mapper.writeValueAsString(signupUser1),
                                jsonHeaders);

                ResponseEntity<String> signupUser1Resp = client.postForEntity(base + "/api/auth/signup", signupUser1Req,
                                String.class);
                ...
                HttpEntity<String> signupUser2Req = new HttpEntity<>(mapper.writeValueAsString(signupUser2),
                                jsonHeaders);

                ResponseEntity<String> signupUser2Resp = client.postForEntity(base + "/api/auth/signup", signupUser2Req,
                                String.class);

                assertThat(signupUser2Resp.getStatusCode().is2xxSuccessful()).isTrue();

                // ---------- SIGN IN USER 1 ----------
                ResponseEntity<String> signinUser1Resp = client.postForEntity(base + "/api/auth/signin", signupUser1Req,
                                String.class);

                assertThat(signinUser1Resp.getStatusCode().is2xxSuccessful()).isTrue();

                Map<String, Object> signinUser1Map = mapper.readValue(signinUser1Resp.getBody(), new TypeReference<>() {
                });

                String tokenUser1 = signinUser1Map.get("token").toString();

                // ---------- SIGN IN USER 2 ----------
                ResponseEntity<String> signinUser2Resp = client.postForEntity(base + "/api/auth/signin", signupUser2Req,
                                String.class);

                assertThat(signinUser2Resp.getStatusCode().is2xxSuccessful()).isTrue();

                Map<String, Object> signinUser2Map = mapper.readValue(signinUser2Resp.getBody(), new TypeReference<>() {
                });
                ...
                HttpEntity<String> expense1Req = new HttpEntity<>(mapper.writeValueAsString(expense1),
                                authHeadersUser1);

                ResponseEntity<String> expense1Resp = client.postForEntity(base + "/api/expenses", expense1Req,
                                String.class);
                ...
                HttpEntity<String> expense2Req = new HttpEntity<>(mapper.writeValueAsString(expense2),
                                authHeadersUser2);

                ResponseEntity<String> expense2Resp = client.postForEntity(base + "/api/expenses", expense2Req,
                                String.class);
                ...
                Map<String, Object> balanceMap = mapper.readValue(balanceResp.getBody(), new TypeReference<>() {
                });
                ...
                List<Map<String, Object>> balances = (List<Map<String, Object>>) balanceMap.get("balances");
                ...
                HttpEntity<String> outsiderSignupReq = new HttpEntity<>(mapper.writeValueAsString(outsiderSignup),
                                jsonHeaders);

                ResponseEntity<String> outsiderSignupResp = client.postForEntity(base + "/api/auth/signup",
                                outsiderSignupReq, String.class);

                assertThat(outsiderSignupResp.getStatusCode().is2xxSuccessful()).isTrue();

                ResponseEntity<String> outsiderSigninResp = client.postForEntity(base + "/api/auth/signin",
                                outsiderSignupReq, String.class);

                assertThat(outsiderSigninResp.getStatusCode().is2xxSuccessful()).isTrue();

                Map<String, Object> outsiderSigninMap = mapper.readValue(outsiderSigninResp.getBody(),
                                new TypeReference<>() {
                                });
                ...
                ResponseEntity<String> forbiddenResp = client.exchange(
                                base + "/api/groups/" + groupId + "/balances",
                                HttpMethod.GET,
